...
/**
 * View for the Control instance, holds Container with every Button, CheckBox
 * and Image
 *
 */
public class ControlView extends JFrame {
	...
	public ControlView() {
		...
		setSize(container.getImageWidth() + 25,
				container.getImageHeight() + 125);
		setResizable(false);
	...
	private void check() {
		...
		timer = new Timer(1000, new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				container.getButton().setText("Connecting");
				container.getButton().setForeground(Color.GRAY);
				if (timerCounter > 0) {

					if (Control.isDroneConnected) {
						container.setInitLabel("Drone connected!", Color.BLUE);
						container.getButton().setForeground(Color.BLUE);
						Control.out.println("Drone connected!");
						container.getButton().setText("Start");
						timer.stop();
					} else {
						container.setInitLabel("Waiting for Video... "
								+ timerCounter + "s", Color.GRAY);
						timerCounter--;
					}

				} else {
					container.setInitLabel(
							"Connection could not be established!", Color.RED);
